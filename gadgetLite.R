...
obj <- MakeADFun(data,parameters,map=list(SIa = factor(rep(NA,4))))
 obj$hessian <- TRUE
...
dat <- stkcomp %>% 
 ...
 ggplot(as.data.frame(dat),aes(length,o))+ geom_line() + geom_line(aes(length,p),lty=2) +
    facet_wrap(~year+step,scale='free_y')
#opt <- do.call("optim",obj)
opt <- nlminb(obj$par,obj$fn,obj$gr)
pl <- obj$env$parList(opt$par)
obj2 <- MakeADFun(data,pl,random="recruits",DLL="gadgetLite")
